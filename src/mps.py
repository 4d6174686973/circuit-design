...
class MPS cumulant
@author: congzlwag
Source: https://github.com/congzlwag/UnsupGenModbyMPS
Only the functions loadMPS and saveMPS were modified. 
"""
...
class MPS:
	...
	### Saving and Loading Functions were rewritten by Mathis Makarski for better handling of directories
	### Anything else kept unchanged

	def saveMPS(self, save_dir) -> None:
		...
		try:
			...
			pass
		save(f'{save_dir}/cutoff.npy', self.cutoff)
		save(f'{save_dir}/loss.npy', asarray(self.Loss))
		save(f'{save_dir}/bond_dim.npy', self.bond_dimension)
		save(f'{save_dir}/train_history.npy', asarray(self.trainhistory))
		save(f'{save_dir}/current_bond.npy', self.current_bond)
		# save all the tensors
		os.makedirs(f'{save_dir}/tensors')
		for i in range(self.space_size):
			save(f'{save_dir}/tensors/arr_' + str(i), self.matrices[i])
	...
	def loadMPS(self, save_dir) -> None:
		"""Loading a MPS from directory save_dir"""
		self.bond_dimension = load(f'{save_dir}/bond_dim.npy')
		self.space_size = len(self.bond_dimension)
		self.trainhistory = load(f'{save_dir}/train_history.npy').tolist()
		self.Loss = load(f'{save_dir}/loss.npy').tolist()
		try:
			self.current_bond = int(load(f'{save_dir}/current_bond.npy'))
		...
		try:
			...
			pass
		try:
			self.cutoff = float(load(f'{save_dir}/cutoff.npy'))
		except:
			pass
		try:
			self.matrices = [np.load(f'{save_dir}/tensors/arr_{b}.npy') for b in range(self.space_size)]
		except:
